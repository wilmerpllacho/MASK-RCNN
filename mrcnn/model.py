...
def compute_backbone_shapes(config, image_shape):
    ...
    assert config.BACKBONE in ["resnet50", "resnet101","resnet18","resnet34"]
    return np.array(
...
def identity_block2(input_tensor, kernel_size, filters, stage, block,
    ...
    conv_name_base = 'ress' + str(stage) + block + '_branch'
    bn_name_base = 'bnn' + str(stage) + block + '_branch'
    ...
    x = KL.Activation('relu', name='res' + str(stage) + block + '_out')(x)
    return x
def conv_block2(input_tensor, kernel_size, filters, stage, block,
               strides=(2, 2), use_bias=True, train_bn=True):
    nb_filter1, nb_filter2 = filters
    conv_name_base = 'resss' + str(stage) + block + '_branch'
    bn_name_base = 'bnnn' + str(stage) + block + '_branch'
    ...
    shortcut = KL.Conv2D(nb_filter2, (1, 1), strides=strides,name=conv_name_base + '1', use_bias=use_bias)(input_tensor)
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    """
    assert architecture in ["resnet50", "resnet101","resnet18","resnet34"]

    if architecture == "resnet18":
        return resnet_graph18(input_image, True, train_bn)
    if architecture == "resnet34":
        return resnet_graph34(input_image, True, train_bn)
    
    # Stage 1
    x = KL.ZeroPadding2D((3, 3))(input_image) #aplica matris a los lados de la imagen
...
def resnet_graph18(input_image, stage5=False, train_bn=True):
    # Stage 1
    ...
    C1 = x = KL.MaxPooling2D((3, 3), strides=(2, 2), padding="same")(x)
    # Stage 2
    x = conv_block2(x, 3, [64, 64], stage=2, block='a', strides=(1, 1), train_bn=train_bn)
    C2 = x = identity_block2(x, 3, [64, 64], stage=2, block='b', train_bn=train_bn)
    # Stage 3
    x = conv_block2(x, 3, [128, 128], stage=3, block='a', train_bn=train_bn)
    C3 = x = identity_block2(x, 3, [128, 128], stage=3, block='b', train_bn=train_bn)
    # Stage 4
    x = conv_block2(x, 3, [256, 256], stage=4, block='a', train_bn=train_bn)
    C4 = x = identity_block2(x, 3, [256, 256], stage=4, block='b', train_bn=train_bn)

    # Stage 5
    if stage5:
        x = conv_block2(x, 3, [512, 512], stage=5, block='a', train_bn=train_bn)
        C5 = x = identity_block2(x, 3, [512, 512], stage=5, block='b', train_bn=train_bn)
    else:
        C5 = None
    return [C1, C2, C3, C4, C5]

def resnet_graph34(input_image, stage5=False, train_bn=True):
    # Stage 1
    x = KL.ZeroPadding2D((3, 3))(input_image)
    x = KL.Conv2D(64, (7, 7), strides=(2, 2), name='conv1', use_bias=True)(x)
    x = BatchNorm(name='bn_conv1')(x, training=train_bn)
    ...
    # Stage 2
    x = conv_block2(x, 3, [64, 64], stage=2, block='a', strides=(1, 1), train_bn=train_bn)
    x = identity_block2(x, 3, [64, 64], stage=2, block='b', train_bn=train_bn)
    C2 = x = identity_block2(x, 3, [64, 64], stage=2, block='c', train_bn=train_bn)
    # Stage 3
    x = conv_block2(x, 3, [128, 128], stage=3, block='a', train_bn=train_bn)
    x = identity_block2(x, 3, [128, 128], stage=3, block='b', train_bn=train_bn)
    x = identity_block2(x, 3, [128, 128], stage=3, block='c', train_bn=train_bn)
    C3 = x = identity_block2(x, 3, [128, 128], stage=3, block='d', train_bn=train_bn)
    # Stage 4
    x = conv_block2(x, 3, [256, 256], stage=4, block='a', train_bn=train_bn)
    x = identity_block2(x, 3, [256, 256], stage=4, block='b', train_bn=train_bn)
    x = identity_block2(x, 3, [256, 256], stage=4, block='c', train_bn=train_bn)
    x = identity_block2(x, 3, [256, 256], stage=4, block='d', train_bn=train_bn)
    x = identity_block2(x, 3, [256, 256], stage=4, block='e', train_bn=train_bn)
    C4 = x = identity_block2(x, 3, [256, 256], stage=4, block='f', train_bn=train_bn)

    # Stage 5
    if stage5:
        x = conv_block2(x, 3, [512, 512], stage=5, block='a', train_bn=train_bn)
        x = identity_block2(x, 3, [512, 512], stage=5, block='b', train_bn=train_bn)
        C5 = x = identity_block2(x, 3, [512, 512], stage=5, block='c', train_bn=train_bn)
    else:
        C5 = None
    return [C1, C2, C3, C4, C5]
############################################################
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    """
    # Load image and mask
    image, mask, class_ids = dataset.load_items(image_id)
